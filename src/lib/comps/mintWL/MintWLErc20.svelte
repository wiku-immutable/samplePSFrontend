<script lang="ts">
	...
	async function mintErc20() {
		try {
			...
			// check whitelist before allowance
			...
			if (!whitelisted) {
				toastStore.trigger(createToastErrorMessage('User is not whitelisted'));
				return;
			}
...
<button on:click={mintErc20} disabled={!$account.isConnected} class="btn variant-filled-secondary">
	{#if loading}
